...
dataset_train = pd.read_csv("data/train-180.csv", index_col=0, delimiter=",", decimal=".")
...
# Importing Test Set
dataset_test = pd.read_csv("data/testing-180.csv", index_col=0, delimiter=",", decimal=".")
cols = list(dataset_test)[1:11]
...
# sc_predict_test.fit_transform(testing_set[:, 0:1])
...
PREDICTION_TRAIN = pd.DataFrame(y_pred_test, columns=['IDR']).set_index(
    pd.Series(datelist_test[2 * n_past + n_future - 2:]))

# show accuracy
score = model.evaluate(X_test, y_test, batch_size=BATCH_SIZE, verbose=0)
# print(score)
# print('Predictions and Actual Gold Prices - LSTM (6 Months / 180 Days):')
...
mse = mean_squared_error(y_test, predictions_test, squared=False)
...
yticksvalue = np.arange(18000000, 22400000, 400000)
ytickslabel = [18, 18.4, 18.8, 19.2, 19.6, 20, 20.4, 20.8, 21.2, 21.6, 22]
plt.yticks(yticksvalue, ytickslabel)
plt.legend(shadow=True)
plt.title('Predictions and Actual Gold Prices - LSTM (6 Months / 180 Days)', family='Arial', fontsize=12)
