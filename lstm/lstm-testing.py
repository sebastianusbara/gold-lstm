...
dataset_train = pd.read_csv("data/train.csv", index_col=0, delimiter=",", decimal=".")
dataset_test = pd.read_csv("data/test.csv", index_col=0, delimiter=",", decimal=".")
...
datelist_test = [dt.datetime.strptime(date, '%b %d %Y').date()
                  for date in datelist_test]
...
# Using multiple features (predictors) for test data
# testing_set = dataset_test.values
# print(list(dataset_test)[1:1])
testing_set = dataset_test.drop(columns=['Date'])
sc = StandardScaler()
testing_set_scaled = sc.fit_transform(testing_set)
print(testing_set_scaled)

sc_predict_test = StandardScaler()
sc_predict_test.fit_transform(testing_set)
...
for i in range(n_past, len(testing_set_scaled) - n_future + 1):
    X_test.append(testing_set_scaled[i - n_past:i, 0:dataset_test.shape[1] - 1])
...
model.add(LSTM(units=64, return_sequences=True, input_shape=(
    n_past, testing_set.shape[1] - 1)))

# Adding 2nd LSTM layer
model.add(LSTM(units=10, return_sequences=False))

# Adding Dropout
model.add(Dropout(0.25, trainable=True))
model.add(Flatten())
...
# Generate list of sequence of days for predictions
datelist_future_test = pd.date_range(datelist_test[-1], periods=n_future, freq='1d').tolist()

# Convert Pandas Timestamp to Datetime object (for transformation) --> FUTURE
datelist_future_test_ = []
for this_timestamp in datelist_future_test:
    datelist_future_test_.append(this_timestamp.date())


# Perform predictions
predictions_future = model.predict(X_test[-n_future:])
predictions_train = model.predict(X_test[n_past:])
...
y_pred_future = sc_predict_test.inverse_transform(predictions_future)
y_pred_train = sc_predict_test.inverse_transform(predictions_train)

PREDICTIONS_FUTURE = pd.DataFrame(y_pred_future, columns=['IDR']).set_index(
    pd.Series(datelist_future_test))
PREDICTION_TRAIN = pd.DataFrame(y_pred_train, columns=['IDR']).set_index(
    pd.Series(datelist_test[2 * n_past + n_future - 1:]))

# show accuracy
score = model.evaluate(X_test, Y_test, batch_size=BATCH_SIZE, verbose=0)
print('Predictions and Actual Gold Prices - LSTM (All Time / 19 Years) - Testing:')
...
#
# yticksvalue = np.arange(0, 28000000, 4000000)
# ytickslabel = [0, 4, 8, 12, 16, 20, 24]
# plt.yticks(yticksvalue, ytickslabel)
# plt.legend(shadow=True)
# plt.title('Predictions and Actual Gold Prices - LSTM (All Time / 19 Years) - Testing', family='Arial', fontsize=12)
