...
for i in range(1, len(training_set_scaled) - n_future + 1):
    X_train.append(training_set_scaled[i - 1:i, 0:dataset_train.shape[1] - 1])
...
PREDICTION_TRAIN = pd.DataFrame(y_pred_train, columns=['IDR']).set_index(
    pd.Series(datelist_train[2 * 1 + n_future - 1:]))
...
plt.plot(actualDate, dataset_train['IDR'],
...
plt.title('Predictions and Actual Gold Prices - LSTM', family='Arial', fontsize=12)
