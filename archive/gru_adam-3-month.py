...
dataset_origin = pd.read_csv("data/train.csv", index_col=0, delimiter=",", decimal=".")
...
n_past = 1000  # Number of past days we want to use to predict the future
...
model.add(GRU(units=64, return_sequences=True, input_shape=(
...
START_DATE_FOR_PLOTTING = '2019-09-01'
START_DATE_FOR_PLOTTING_ALT = date(year=2019, month=9, day=1)

selectedDate = pd.DataFrame(datelist_train)
selectedDataset = pd.concat([selectedDate, dataset_origin], axis=1)
selectedDataset = selectedDataset.set_index(0)
actualDataset = selectedDataset.loc[START_DATE_FOR_PLOTTING_ALT:]
...
plt.plot(actualDataset.index, actualDataset['IDR'],
...
plt.title('Predictions and Actual Gold Prices - GRU + ADAM Learning Rate 0.01', family='Arial', fontsize=12)
plt.ylabel('Gold Price Value', family='Arial', fontsize=10)
